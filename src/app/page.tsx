...
import { fetchFeaturedCars, fetchCarOfTheWeek } from "../lib/sanityClient";
import CarCard from "../components/CarCard";
import Image from "next/image";

export default async function Home() {
  const featuredCars = await fetchFeaturedCars();
  const carOfTheWeek = await fetchCarOfTheWeek();
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Car of the Week */}
      {carOfTheWeek && (
        <section className="bg-gradient-to-br from-navy to-[#051118] py-16 md:py-20">
          <div className="mx-auto max-w-6xl px-4">
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 overflow-hidden">
              <div className="grid lg:grid-cols-2 gap-8 items-center">
                ...
                <div className="relative h-80 lg:h-96">
                  <Image
                    src={carOfTheWeek.images?.[0] || "/placeholder-car.svg"}
                    alt={carOfTheWeek.title}
                    fill
                    className="object-cover"
                  />
                  {/* Deal Tag Badge */}
                  {carOfTheWeek.dealTag && (
                    <div className="absolute top-4 left-4 bg-gold px-4 py-2 rounded-full font-bold text-white text-sm uppercase tracking-wide shadow-lg">
                      {carOfTheWeek.dealTag.replace('-', ' ')}
                    </div>
                  )}
                </div>
                ...
                <div className="p-8 lg:pr-12">
                  <div className="inline-block bg-gold/20 text-gold px-4 py-1 rounded-full text-sm font-semibold mb-4">
                    🔥 Car of the Week
                  </div>
                  <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
                    {carOfTheWeek.title}
                  </h2>
                  <div className="flex items-center gap-4 text-cream/80 mb-6">
                    <span>{carOfTheWeek.year}</span>
                    <span>•</span>
                    <span>{carOfTheWeek.mileage?.toLocaleString()} km</span>
                    <span>•</span>
                    <span className="capitalize">{carOfTheWeek.condition}</span>
                  </div>
                  <div className="mb-6">
                    <div className="text-4xl font-bold text-white mb-2">
                      ₦{carOfTheWeek.price.toLocaleString()}
                    </div>
                    <div className="text-gold font-medium">
                      Pay from ₦{Math.ceil((carOfTheWeek.price * 0.5) / 6).toLocaleString()}/month
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-4">
                    <Link
                      href={`/car/${carOfTheWeek.slug}`}
                      className="inline-block text-center rounded-full bg-gold px-8 py-4 font-semibold text-white hover:bg-gold-dark transition-all hover:scale-105 shadow-lg"
                    >
                      View Details
                    </Link>
                    <a
                      href={`https://wa.me/2348127230704?text=Hi EJ, I'm interested in the ${encodeURIComponent(carOfTheWeek.title)}`}
                      className="inline-flex items-center justify-center rounded-full border border-white/20 px-8 py-4 font-semibold text-white hover:bg-white/10 transition-all"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <span className="mr-2 text-[#25D366]">
